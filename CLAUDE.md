...
| `tool-dialects.ts` | Recovering tool calls a HOST failed to parse. Some free hosts return the model's native dialect as assistant TEXT instead of `tool_calls`, which reached the client as markup it treated as a final answer — no `tool_calls` ⇒ `end_turn` ⇒ zero `tool_use` ⇒ the validator passes ⇒ repair never engages. Parsing, not inference: a CLOSED envelope set, and prose naming a tool stays prose. An unparseable envelope yields `detected`, and the caller fails clean so failover reaches a host that parses. Both paths: buffered in `openAiResponseToAnthropic`, streaming in `dialect-stream.ts` (stream text through, withhold from the first marker, recover at stream end; a truncated envelope becomes a mid-stream SSE error). ⚠ The OpenAI front's direct passthrough is still uncovered — see [docs/tool-call-dialect-leak.md](docs/tool-call-dialect-leak.md). |
