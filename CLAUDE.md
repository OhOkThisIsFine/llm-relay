...
| File | Responsibility |
|---|---|
| `os-keyring.ts` | OS-custodied KEK creation and recovery through DPAPI, Keychain, libsecret, or scrypt passphrase mode; secret bytes stay on captured stdin/stdout and child failures are sanitized. |
| `keystore.ts` | Versioned encrypted credential store: lazy KEK unwrap, AES-256-GCM entries with AAD, non-secret descriptors, lifecycle mutations, zeroization on lock, and atomic restrict-before-publish writes. |
