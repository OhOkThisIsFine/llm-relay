...
| `keystore.ts` | Versioned encrypted credential store: KEK-verifier-checked lazy unwrap memoized process-wide for success or failure, AES-256-GCM rows bound to version/provider/id/envName, salted keyed fingerprints, non-secret descriptors/status, lifecycle mutations, zeroization, and atomic restrict-before-publish writes. Existing-but-unparseable or degraded stores refuse every mutation byte-preservingly: credential rows are the operator's only keys, unlike re-learnable target facts, so degrade-to-fresh is unsafe. |
