...
| `dashboard-snapshot.ts` | The bounded read-only projection from the accounting store to dashboard views. Knows only the persisted read model — quota/cooldown facts are injected as one already-captured snapshot, so the projection neither owns the store nor reaches into live provider/breaker state. Projects real spend totals from the aggregates (summary, every dimension row, request rows/detail), keeping unpriced cells amount-null — "Unpriced", never "$0" — and surfacing the lower-bound marker when `partiallyPricedRequests` > 0. |
