...
| `dashboard-contract.ts` | The versioned server-safe wire contract for the analytics dashboard (`dashboard.snapshot.v1`, media type, query spellings) — platform-free by rule, so route/auth/retention/pricing decisions stay with their owners. All request/response bounds are explicit constants here. `SpendTotalsV1` carries the four priced cells plus `unpricedRequests` AND `partiallyPricedRequests` (>0 ⇒ every amount is a LOWER BOUND); the producer ships with the validator, so the field is required, not tolerated-absent. |
...
| `dashboard-snapshot.ts` | The bounded read-only projection from the accounting store to dashboard views. Knows only the persisted read model — quota/cooldown facts are injected as one already-captured snapshot (built per snapshot read by `createAvailabilityProducer` in `availability-snapshot.ts`), so the projection neither owns the store nor reaches into live provider/breaker state. Projects real spend totals from the aggregates (summary, every dimension row, request rows/detail), keeping unpriced cells amount-null — "Unpriced", never "$0" — and surfacing the lower-bound marker when `partiallyPricedRequests` > 0. |
