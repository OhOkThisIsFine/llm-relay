...
- **⚠⚠ A HEDGE puts TWO attempts in flight, and everything the walk touches had a hidden
  ...
  - ⚠ **A hedge LOSER is `recordAbandoned`, never `record`.** A `cancelled` outcome sets `#stopped`
    ...
    on the breaker (cancellation returns before the provenance table), so **hedging HIDES the
    slowness it routes around** — a stated cost, tolerable only because a non-hedged request still
    cools a slow deployment for as long as it wasted (v0.65.3).
