...
| `setup-claude.ts` | Configuration generator for Claude Desktop MCP dispatch (`claude_desktop_config.json`) and directly routed Claude CLI wrappers. Desktop setup also removes only the exact stale proxy env values written through v0.68.4; Desktop pins its own API base, so those values never routed its sessions. |
