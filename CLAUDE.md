...
| `config-types.ts` | Shared configuration, provider, and routing type definitions and constants (`EFFORT_LEVELS`, `Config`, `Routing`, `ProviderConfig`, `ResolvedTarget`, etc.) decoupled from runtime loaders. |
...
| `storage/` | Centralized atomic JSON persistence and managed in-memory state store (`json-store.ts`). Implements nonced temp-file atomic writes (`atomicWriteJsonSync`), safe reads with schema validation guards (`safeReadJsonSync`), and debounced `WriteBehindTimer` persistence (`JsonStore<T>`). |
...
| `candidate-runner.ts` | Candidate walk execution, hedging race, usability ranking (`orderByUsability`), retry classification, and attempt lifecycle management. |
| `stream-pipeline.ts` | Pure stream I/O, watchdog wrappers (`withStallWatchdog`), fail-closed responses (`failClosed`), body buffering (`readBody`), and SSE frame builders. |
