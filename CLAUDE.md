...
| `dispatch.ts` | The dispatch ladder (`GET/POST /dispatch`, `llm-relay dispatch`) — which LANE a host should hand a whole delegated task to, in order, with tier selection (`?tier=`), host override (`?lane=`), walk-past (`?after=`) and host-reported exhaustion (`POST {"exhausted"}`). `routing.ladders.<tier>` supports different CLI models for reasoning/coding/fast; the legacy `routing.ladder` remains valid. An exhaustion report may carry `outcome: "rate_limited"` (15m default) or `"quota_exhausted"` (1h default) and a vendor-stated `retryAfterMs` that beats both (`OUTCOME_DEFAULT_MS`); the relay still never invents the signal. A `cli` rung may declare `env` (string = set, `null` = unset — both needed for a relay-routed `claude -p` child: base URL set, nested-session vars unset), surfaced on `invoke.env` and rendered by the CLI per shell; the task placeholder is never substituted into env values. Distinct from `routing.subagents`, which routes one HTTP turn. **The request path never spawns a `cli` rung** — it owns the order, the host executes; the only lane-spawn sites are the operator probe and the background cadence (owner amendment 2026-08-29, see the ladder gotcha). Exhaustion state is per-Config in memory, mirrored to disk by `dispatch-exhaustion-persistence.ts` (future-only restore), with key-level ops (`markExhaustedKey`/`clearExhaustedKey`/`exportExhaustedRows`/`restoreExhaustedRows`) so the quota probe can write the bucket it tested without a rung lookup. `resolveAutoSpec()` resolves the `auto` model name to the first ready relay rung in the ladder for a tier (defaulting to medium) or falls back to `routing.default`. Every lane also carries an advisory `stats` field (calls/successes/failures/timeouts, median wall-clock, last run) filled from `dispatch-lane-stats.ts` for rungs that ran under this config and omitted otherwise — rendered by `dispatch_lanes`/`llm-relay dispatch` via `formatLaneStats`, and never changing `state`, `next`, or the ladder order. ⚠ **`DispatchView.order` (2026-09-06) is the ONE definition of SELECTION ORDER** — the lane ids that may be tried, best first, with `next` being `order[0]` resolved. It exists so the order has one owner: a dispatch WALK iterates the same list, and the alternative (a walking caller re-deriving the order from `ladder`) is the two-definitions-of-one-policy shape this repository's history warns about more than any other. `ladder` itself stays in CONFIG order, because `position` is documented stable and a reader needs the configured ladder rather than a re-sorted one; unselectable rungs are absent from `order` entirely. ⚠ Ranking is pinned → undemoted → demoted, from `lane-affinity.ts`, and the sort is STABLE so the operator's own ladder order remains the tie-break within a band. A lane carrying BOTH memories ranks as PINNED: that state is reachable (answer, get pinned, miss a budget later) and the pin is the more recent evidence, since recording either retracts the other. ⚠ A `?lane=` override sets `order` to that ONE lane, so a walking caller honours the override too — walking past it would defeat it as silently as ignoring it. ⚠⚠ **`attemptBudget` (2026-09-08, owner direction) is each lane's WALK BUDGET, derived from that lane's OWN recorded runs** — the 80th percentile of its rolling window, `basis: "history"`; the flat `attemptMs` at `basis: "floor"` below `attemptMinSamples`. It is computed HERE and carried on the lane because the daemon is where the history lives and the `llm-relay mcp` child that runs the walk holds none; the child reads `lane.attemptBudget?.ms` and falls back to its own flat figure only for a view that carries none (a daemon older than the field, or the local fallback). ⚠ **This is the owner's request-path mechanism applied to lanes — the METHOD, never the NUMBERS.** The shipped v0.74.0 default was a flat 90 s, and measured against the live store the same day it sat BELOW the median run of two of the three working lanes and below the free pool's median by a factor of six, so the relay would have abandoned the free pool on nearly every dispatch — the opposite of the walk's purpose. ⚠ 0.8 rather than the request path's 0.95, because at p90/p95 the slowest lane's figure IS its own timeout and a budget there could never fire (live p80 at the time of the decision: 165 s, 224 s, 1383 s against 1800 s timeouts). ⚠⚠ **Those figures are PERISHABLE and MACHINE-LOCAL, and must not be read as a reproducible measurement.** They are a snapshot of one operator's rolling window, which real traffic overwrites: an independent audit re-read the same file hours later and found two of the three unchanged and the third already moved (free-pool p50 583 s → 302 s, p80 1383 s → 1207 s) because dispatches continued. ⚠ The DECISION is unaffected and that is the point of recording the drift rather than the number alone — free-pool's median is still more than three times the flat 90 s it replaced, so the finding that a flat budget abandons the slow lane holds under both readings. Re-measure from `~/.llm-relay/dispatch-lane-stats.json` before quoting a figure; never cite these as though a later reader could reproduce them. ⚠ The TOKEN-normalised rung does NOT carry over: a walk budget fires before any answer, so there is no output token to normalise by — the same reason `hedge-trigger.ts`'s per-token rung is inert on the hedge path. ⚠ Too little history ⇒ the FLAT figure, never a quantile over two samples (unmeasured is no opinion, never slow), and the result is clamped never to fall BELOW that flat figure — one fast streak must not make the relay permanently impatient with a lane. |
...
| `stream-pipeline.ts` | Pure stream I/O, watchdog wrappers (`withStallWatchdog`), fail-closed responses (`failClosed`), body buffering (`readBody`), and SSE frame builders. Owns `BODY_TOO_LARGE_CODE`, the tag `readBody` sets when it refuses an oversized body, and `bodyReadStatus` — 413 for that tag, 400 for anything else — the data plane's classifier since 2026-09-04 (contract review DR-005): `server.ts` used to decide 413 with `message.includes("too large")` while the dashboard route already compared the code, and this module imported that code FROM `dashboard-routes.ts`, a data-plane module depending on the dashboard. The dependency now points the other way and `dashboard-routes.ts` re-exports the tag. |
...
| `routes/openai-front.ts` | OpenAI frontend protocol handler: Chat Completions (`/v1/chat/completions`) and Responses (`/v1/responses`) endpoints (`openAiFrontPath`). |
...
| `setup-claude.ts` | Configuration generator for Claude Desktop MCP dispatch (`claude_desktop_config.json`) and directly routed Claude CLI wrappers. Both targets install the custom `relay` agent definition (`~/.claude/agents/relay.md`) so Claude Workflow and Agent tool calls can dispatch to llm-relay lanes. Desktop setup also removes only the exact stale proxy env values written through v0.68.4; Desktop pins its own API base, so those values never routed its sessions. ⚠ **`RELAY_AGENT_TEMPLATE` pins NO model as of v4 (2026-09-04, owner direction: "I don't want to hard code a model name")** — `model: inherit`, not an omitted `model:` line: confirmed against `code.claude.com/docs/en/sub-agents.md` that an omitted field can fall through to the `CLAUDE_CODE_SUBAGENT_MODEL` env var before ever reaching the calling session's model, while `inherit` selects it directly. Measured the same day: `model: haiku` answered a trivial echo task itself (no provenance line) while a realistic task correctly dispatched; `model: sonnet` obeyed on both. `installRelayAgent`'s marker-prefix ownership test (any versioned `<!-- llm-relay:relay-agent vN -->`) upgrades an older file in place. Codex gets the analogous fix in `scripts/install-skill.mjs` — see `scripts/CLAUDE.md` — because that script, not this one, already owns `~/.codex/agents/`; the two templates carry the same pass-through contract, ported to each host's own subagent-file schema and MCP tool-naming convention. |
...
- **Accounting metering, not custody:** per credential and deployment, record used, left, and
  ...
  amendment 2026-08-30, `docs/hedged-attempts-design-2026-08-30.md` §7). The exception is narrow and
...
- **Health DEMOTES candidates; it never drops them.** `orderByUsability()` returns every candidate,
  ordered live → slow → credential-faulted → cooling. 2026-09-03, owner-approved: latency-demoted
  members form a `slow` band above the failure bands, because measured walks reached the only
  answering member after 6 to 9 failing ones. Within the cooling band, candidates are ordered by
