// Live: full pipeline on NIM ALONE (no Anthropic key). A stub Anthropic-format
// backend emits a malformed (flattened) tool call; the validator flags it, and a
// REAL NIM reshaper (openai-kind transport) fixes it — all inside dist/cli.js.
//
...
// Stub Anthropic-format backend: returns a tool_use with FLAT args {city,unit} —
// a common weak-model failure against a schema that requires them nested under
// `location`.
const stub = createServer((req, res) => {
  ...
  req.on("end", () => {
    ...
    res.end(JSON.stringify({
      id: "msg_stub", type: "message", role: "assistant", model: "stub-weak-model",
      stop_reason: "tool_use",
      content: [{ type: "tool_use", id: "c1", name: "get_weather", input: { city: "Paris", unit: "C" } }],
    }));
  });
});
...
writeFileSync(cfgPath, JSON.stringify({
  ...
  backend: { base: `http://127.0.0.1:${stubPort}` },
