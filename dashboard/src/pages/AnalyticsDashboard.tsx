...
import { PanelCoverage, SpendCells, TokenCells } from "../components/ProjectionMetadata.js";
import { SummaryCards } from "../components/SummaryCards.js";
import { currencyMicrousd, duration, number, percent, stamp, utcBucketLabel } from "../formatters.js";
import { quotaHeadroom } from "../view-model.js";
...
function SpendPanel({ snapshot }: Readonly<{ snapshot: SnapshotV1 }>): ReactElement {
  ...
  const rows: ReadonlyArray<readonly [string, SpendTotalsV1[keyof Omit<SpendTotalsV1, "unpricedRequests">]]> = [["Provider-published / reported", spend.providerPublishedReported], ["Provider-published / estimated", spend.providerPublishedEstimated], ["Reference / reported", spend.referenceReported], ["Reference / estimated", spend.referenceEstimated]];
  return <section className="panel" aria-labelledby="spend-heading"><h2 id="spend-heading">Spend detail</h2><PanelCoverage label="Spend" value={coverageFor(snapshot, "spend")} /><ResponsiveTable caption="Spend cells are intentionally not blended" headers={["Cell", "Amount", "Price source", "Token basis", "Measurement source", "Observed"]}>{rows.map(([label, cell]) => <tr key={label}><th scope="row" data-label="Cell">{label}</th><td data-label="Amount">{currencyMicrousd(cell.amountMicrousd)}</td><td data-label="Price source">{cell.priceSource}</td><td data-label="Token basis">{cell.tokenBasis}</td><td data-label="Measurement source">{cell.source}</td><td data-label="Observed">{stamp(cell.observedAt)}</td></tr>)}<tr><th scope="row" data-label="Cell">Unpriced requests</th><td data-label="Amount" colSpan={5}>{number(spend.unpricedRequests)}</td></tr></ResponsiveTable></section>;
}
...
function DimensionPanel({ snapshot, panel, title, rows, label }: Readonly<{ snapshot: SnapshotV1; panel: "provider" | "model" | "client" | "credential"; title: string; rows: readonly DimensionRowV1[]; label(row: DimensionRowV1): string }>): ReactElement {
  return <section className="panel" aria-labelledby={panel + "-heading"}><h2 id={panel + "-heading"}>{title}</h2><PanelCoverage label={title} value={coverageFor(snapshot, panel)} /><ResponsiveTable caption={title + " breakdown"} headers={["Dimension", "Requests", "Attempts", "Served", "Errors", "Cancelled", "Success", "Latency", "Commit", "Coverage", "Tokens", "Spend"]}>{rows.length === 0 ? <tr><td colSpan={12}>No matching measurements.</td></tr> : rows.map((row) => <tr key={label(row)}><th scope="row" data-label="Dimension">{label(row)}</th><td data-label="Requests">{number(row.requests)}</td><td data-label="Attempts">{number(row.attempts)}</td><td data-label="Served">{number(row.served)}</td><td data-label="Errors">{number(row.errored)}</td><td data-label="Cancelled">{number(row.cancelled)}</td><td data-label="Success">{percent(row.successRate)}</td><td data-label="Latency">{duration(row.avgLatencyMs)}</td><td data-label="Commit">{duration(row.avgCommitMs)}</td><td data-label="Coverage">{row.coverage}</td><td data-label="Tokens"><TokenCells tokens={row.tokens} /></td><td data-label="Spend"><SpendCells spend={row.spend} /></td></tr>)}</ResponsiveTable></section>;
}
...
function ErrorPanel({ snapshot }: Readonly<{ snapshot: SnapshotV1 }>): ReactElement { return <section className="panel" aria-labelledby="errors-heading"><h2 id="errors-heading">Normalized errors</h2><PanelCoverage label="Errors" value={coverageFor(snapshot, "errors")} /><ResponsiveTable caption="Normalized error distribution" headers={["Failure kind", "Outcome", "Requests"]}>{snapshot.errors.length === 0 ? <tr><td colSpan={3}>No matching measurements.</td></tr> : snapshot.errors.map((row) => <tr key={row.failureKind + "-" + row.outcome}><th scope="row" data-label="Failure kind">{row.failureKind}</th><td data-label="Outcome">{row.outcome}</td><td data-label="Requests">{number(row.requests)}</td></tr>)}</ResponsiveTable></section>; }
function QuotaPanel({ snapshot }: Readonly<{ snapshot: SnapshotV1 }>): ReactElement { return <section className="panel" aria-labelledby="quotas-heading"><h2 id="quotas-heading">Quota headroom</h2><PanelCoverage label="Quotas" value={coverageFor(snapshot, "quotas")} /><ResponsiveTable caption="Quota measurements and derived headroom" headers={["Credential", "Provider", "Deployment", "Axis", "Period", "Limit", "Limit basis", "Remaining", "Remaining basis", "Local used", "Local basis", "Headroom", "Resets", "Observed"]}>{snapshot.quotas.length === 0 ? <tr><td colSpan={14}>No matching measurements.</td></tr> : snapshot.quotas.map((row) => <tr key={quotaRowKey(row)}><th scope="row" data-label="Credential">{row.label} ({row.credentialId})</th><td data-label="Provider">{row.provider}</td><td data-label="Deployment">{safe(row.deployment)}</td><td data-label="Axis">{row.axis}</td><td data-label="Period">{row.period}</td><td data-label="Limit">{number(row.limit)}</td><td data-label="Limit basis">{safe(row.limitBasis)}</td><td data-label="Remaining">{number(row.remaining)}</td><td data-label="Remaining basis">{safe(row.remainingBasis)}</td><td data-label="Local used">{number(row.localUsed)}</td><td data-label="Local basis">{safe(row.localUsedBasis)}</td><td data-label="Headroom">{percent(quotaHeadroom(row))}</td><td data-label="Resets">{stamp(row.resetsAt)}</td><td data-label="Observed">{stamp(row.observedAt)}</td></tr>)}</ResponsiveTable></section>; }
function CooldownPanel({ snapshot }: Readonly<{ snapshot: SnapshotV1 }>): ReactElement { return <section className="panel" aria-labelledby="cooldowns-heading"><h2 id="cooldowns-heading">Cooldowns</h2><PanelCoverage label="Cooldowns" value={coverageFor(snapshot, "cooldowns")} /><ResponsiveTable caption="Normalized cooldowns" headers={["Credential", "Provider", "Deployment", "Reason", "Until", "Observed"]}>{snapshot.cooldowns.length === 0 ? <tr><td colSpan={6}>No matching measurements.</td></tr> : snapshot.cooldowns.map((row) => <tr key={cooldownRowKey(row)}><th scope="row" data-label="Credential">{row.credentialId}</th><td data-label="Provider">{row.provider}</td><td data-label="Deployment">{safe(row.deployment)}</td><td data-label="Reason">{row.reason}</td><td data-label="Until">{stamp(row.until)}</td><td data-label="Observed">{stamp(row.observedAt)}</td></tr>)}</ResponsiveTable></section>; }
function RecentPanel({ snapshot, onDetail, rememberTrigger }: Readonly<{ snapshot: SnapshotV1; onDetail(requestId: string): void; rememberTrigger(target: HTMLElement): void }>): ReactElement { return <section className="panel" aria-labelledby="recent-heading"><h2 id="recent-heading">Recent requests</h2><PanelCoverage label="Recent requests" value={coverageFor(snapshot, "recent")} /><ResponsiveTable caption="Bounded recent request projection" headers={["Request", "Occurred", "Client", "Attribution", "Outcome", "Failure", "Attempts", "Latency", "Commit", "Provider", "Model", "Credential", "Tokens", "Spend", "Repair included", "Details"]}>{snapshot.recentRequests.length === 0 ? <tr><td colSpan={16}>No matching measurements.</td></tr> : snapshot.recentRequests.map((row) => <tr key={row.requestId}><th scope="row" data-label="Request">{row.requestId}</th><td data-label="Occurred">{stamp(row.occurredAt)}</td><td data-label="Client">{safe(row.client)}</td><td data-label="Attribution">{row.attribution}</td><td data-label="Outcome">{row.outcome}</td><td data-label="Failure">{safe(row.failureKind)}</td><td data-label="Attempts">{number(row.attemptCount)}</td><td data-label="Latency">{duration(row.latencyMs)}</td><td data-label="Commit">{duration(row.commitMs)}</td><td data-label="Provider">{safe(row.provider)}</td><td data-label="Model">{safe(row.model)}</td><td data-label="Credential">{safe(row.credentialId)}</td><td data-label="Tokens"><TokenCells tokens={row.tokens} /></td><td data-label="Spend"><SpendCells spend={row.spend} /></td><td data-label="Repair included">{row.repairIncluded ? "Yes" : "No"}</td><td data-label="Details"><button type="button" onClick={(event) => { rememberTrigger(event.currentTarget); onDetail(row.requestId); }}><span className="sr-only">View request </span>{row.requestId}</button></td></tr>)}</ResponsiveTable></section>; }
