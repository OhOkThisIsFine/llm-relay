...
describe("dashboard accessibility and truthfulness", () => {
  ...
  it("keeps labelled table data usable at 320, 768, and 1280px; themes and reduced motion stay self-hosted", async () => {
    ...
    const media = vi.fn().mockReturnValue({ matches: true, addEventListener: vi.fn(), removeEventListener: vi.fn() }); vi.stubGlobal("matchMedia", media); expect(window.matchMedia("(prefers-reduced-motion: reduce)").matches).toBe(true); expect(container.querySelectorAll("table").item(0)?.getAttribute("style")).toBeNull();
  });
