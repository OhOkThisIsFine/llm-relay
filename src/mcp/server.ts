...
export class McpDispatchServer {
  ...
  /**
   * A killed job is adopted synchronously by `LaneJobStore`, but only the server owns the git reader.
   * Complete the report here by comparing the journaled start with the tree as it exists when this
   * restarted server adopts the job. That timing is stated explicitly: it can include edits made
   * after the old process died, so it must never be described as an exact death-time snapshot.
   */
