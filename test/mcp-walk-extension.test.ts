...
function harness(opts: {
  ...
  let treeReads = 0;
  const treeSnapshot: TreeSnapshotReader = async () => {
    treeReads += 1;
    return readings === undefined ? null : (readings.shift() ?? null);
  };
  ...
  return { call, started, headers, asked, treeReads: () => treeReads };
}
...
describe("the walk stops a lane only when it is idle", () => {
  ...
  it("dispatch_status is observational and never consumes relay, CPU, or tree liveness evidence", async () => {
    ...
    const h = harness({
      lanes: [cliLane("slow"), cliLane("next")],
      scripts: {
        slow: { answersAfterMs: 10 * IDLE_MS, pids: [909] },
    ...
    await vi.advanceTimersByTimeAsync(IDLE_POLL_MS);
    const before = { relay: h.asked.length, cpu: cpuReads, tree: h.treeReads() };
    expect(before.relay).toBeGreaterThan(0);
    expect(before.cpu).toBeGreaterThan(0);
    expect(before.tree).toBeGreaterThan(0);

    for (let i = 0; i < 3; i += 1) {
      const status = h.call("dispatch_status", { jobId });
      await status.done;
      expect(status.text()).toContain("activity: quiet");
      expect(status.text()).toContain("walk-verdict: keep-running");
    }
    expect({ relay: h.asked.length, cpu: cpuReads, tree: h.treeReads() }).toEqual(before);
  });
  ...
  it("keeps a silent non-relay lane while its owned process tree accumulates CPU time", async () => {
    ...
    const h = harness({
      lanes: [cliLane("slow"), cliLane("next")],
      scripts: { slow: { answersAfterMs: 10 * IDLE_MS, pids: [101] }, next: { answersAfterMs: 100 } },
      cpu: (pids) => {
        seenPids.push([...pids]);
        cpuMs += 1_000;
        return cpuMs;
  ...
  it("does not treat a flat process CPU reading as activity", async () => {
    const h = harness({
      lanes: [cliLane("slow"), cliLane("next")],
      scripts: { slow: { answersAfterMs: 10 * IDLE_MS, pids: [202] }, next: { answersAfterMs: 100 } },
      cpu: () => 5_000,
    });
    const text = await finish(h, {});
    expect(h.started).toEqual(["slow", "next"]);
  ...
  it("treats an unavailable process CPU reading as no signal", async () => {
    const h = harness({
      lanes: [cliLane("slow"), cliLane("next")],
      scripts: { slow: { answersAfterMs: 10 * IDLE_MS, pids: [303] }, next: { answersAfterMs: 100 } },
      cpu: () => null,
    });
    await finish(h, {});
    expect(h.started).toEqual(["slow", "next"]);
