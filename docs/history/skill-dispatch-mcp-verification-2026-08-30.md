...
The 2026-08-16 ledger (`docs/history/rejection-ledger-2026-08-16.md:27`) rejected a "hand-rolled MCP
...
2. **Long lanes have no representation.** agy runs at `--print-timeout 30m`. A 30-minute
   ...
   plus a status store — the shape `docs/history/suggestion-review-2026-08-04.md:87-93` already rejected.
3. **It is precisely the future async spawn site the repo warned about.**
   `docs/history/quota-reprobe-design-2026-08-29.md:200-203` records two Windows spawn traps that each cost
...
- **MCP over HTTP/SSE on the daemon** — a **violation**. The `/ping` precedent is directly on point:
  `docs/history/quota-reprobe-design-2026-08-29.md:98-101` records that a cadence hook firing from
