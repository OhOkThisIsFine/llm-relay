...
| 10 | Estimated output tokens | **implemented (M4, `32f31c3`)** | attempt-scoped `observeUsage()` producer on both fronts and reshaper (`src/usage-observer.ts:99-103,834`; `src/backend.ts:590,702,1379`; `src/reshaper.ts:423`) counts model-authored text, thinking/reasoning and whole tool-argument JSON, skips base64 and nulls the whole estimate on any taint; the shared chars/4 convention is `estimateTokensFromCharacters()` (`src/metadata.ts:132`), and `accountingTokens()` writes `estimated.outputTokens` method `relay_estimate` separately from reported usage (`src/server.ts:616-625`) | — |
...
| 2 Windows and rollups | **implemented** in the store; **CLI/endpoint surface absent** | 6; 7 partial; 10 delivered (M4) | windows via day shards + lifetime months (`accounting-store.ts:751-794,1055-1082`); by-client dimension on rows (`:948,1033`); latency/commit distributions (`:176-185,481-500`) |
...
| M3 | Cooldown-clear only behind control token + admission checks | **delivered 2026-08-24 (commit `1ee1ad2`)** — `POST /cooldowns/clear` and its CLI join the shared control-route admission path, reject widened scope grammar and retract only cooling state (`server.ts:131-195`; `routes/admin.ts:194-227`; `cooldown-clear.ts:47-77`; CLI `cli.ts:995-1099`) | **matches** |
| M4 | Estimated output: decide after measuring absence | **delivered 2026-08-24 (commit `32f31c3`)** after the owner waived the evidence gate — the usage observer produces the separate attempt-scoped estimate; serve estimates are final-wire-gated, repair estimates unconditional, and any parse/bound taint leaves the whole attempt unknown (`usage-observer.ts:99-103,834`; `server.ts:616-625`) | **matches the superseding 2026-08-23 owner decision** |
...
| P1 | Custody Windows/macOS | **DELIVERED 2026-08-24 (custody sprint, queued v0.45.0)** — Windows DPAPI live-verified incl. fresh-process unwrap; Linux passphrase mode CI-real; macOS/`secret-tool` injected-double only (stated residual). Plan: `docs/history/custody-sprint-plan-2026-08-24.md` D2 | **delivered in-build** |
...
- **DELIVERED 2026-08-24 (`32f31c3`) — Gap 10 / M4.** The owner-waived producer now records attempt-scoped estimated output separately from reported usage. See §7.
- **DELIVERED 2026-08-24 (`1ee1ad2`) — M3.** The cooldown-clear mutation shipped with the control token plus the same Origin/content-type/Host admission checks as `/offload` and `/dispatch` (`quota-metering-spec-2026-08-16.md` §6.2). See §7.
...
- **Dashboard/`cost` coverage-partial semantics — DELIVERED 2026-08-23 (`50e8233`).** Not carried
  ...
  HANDOFF.md §6.
- **Gap 10 / M4 — DELIVERED 2026-08-24 (`32f31c3`, v0.44.0).** The attempt-scoped producer lives at
  the usage observer, covering both fronts and reshaper calls by construction. It counts only
  model-authored text, thinking/reasoning and whole tool-argument JSON (unlike the request-side
  values-only walk), skips base64, and uses the shared chars/4 convention; `estimated.outputTokens`
  method `relay_estimate` stays in its own cell, never summed into or shadowing reported usage.
  Serve estimates are gated on final-wire commit, repair attempts are metered unconditionally, and
  invalid UTF-8, oversized frames, unparseable argument JSON or bound breaches null the WHOLE
  attempt's estimate, never 0. Consequence: the estimated-basis `usedInWindow` scalar now completes
  to input+output, so an operator tpm/tpd `limits.hard` cap refuses sooner and the
  `derived:configured` demotion rung moves; the pre-M4 input-only figure was a systematic undercount
  that fired the operator's own declared cap late.
...
- **M3 — DELIVERED 2026-08-24 (`1ee1ad2`, v0.44.0).** `POST /cooldowns/clear` and
  `llm-relay cooldowns clear <provider>[/<model>] [--credential <label>]` shipped with the §6.2
  security precondition: the route joins `CONTROL_ROUTES`, so Host/present-Origin/content-type and
  the control token run through the same `admissionFailure()` path as `/offload` and `/dispatch`.
  Both grammars fail closed: an unknown body key is a 400 naming it; the CLI enforces exact arity
  plus a flag allow-list and sends nothing on malformed input. It clears only breaker cooldowns
  (including Retry-After and unexplained-429 escalation), credential faults, quota cooldowns and the
  three cooling condition facts; measurement and eviction facts, failure/stability history and the
  accounting store remain. No running relay means exit 1 — there is no file fallback.
...
- **P1 / custody — DELIVERED 2026-08-24 (queued for v0.45.0).** Stage 3 of
  `docs/history/credential-fleet-design-2026-08-16.md` shipped as six commits (`a83eef8`..`b7129fa`), three
  adversarially reviewed packets; P1's platform-coverage question resolved in-build
  (`docs/history/custody-sprint-plan-2026-08-24.md` D2). Residuals and deliberately-unfixed review
