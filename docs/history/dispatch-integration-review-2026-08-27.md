...
### 2.1 Launcher shim — `C:\Users\<user>\.llm-relay\bin\lane-launch.ps1`
...
```
pwsh -NoProfile -ExecutionPolicy Bypass -File C:\Users\<user>\.llm-relay\bin\lane-launch.ps1 C:\Users\<user>\AppData\Local\agy\bin\agy.exe -p '{task}' --model <id> --output-format json
```
...
`codex` resolves to `C:\Users\<user>\AppData\Roaming\npm\codex.ps1`, an npm shim — a `.ps1` cannot be
...
`C:\Users\<user>\.gemini\antigravity-cli\settings.json` now reads:
...
1. `where agy` resolves to one native executable,
   `C:\Users\<user>\AppData\Local\agy\bin\agy.exe` (186,767,512 bytes). **There is no shim** — that
...
```bash
pwsh -NoProfile -File C:\Users\<user>\.llm-relay\bin\lane-launch.ps1 C:\Users\<user>\AppData\Local\agy\bin\agy.exe -p "Read C:\Code\llm-relay\package.json and reply with only its version field." --output-format json
```
...
```bash
pwsh -NoProfile -File C:\Users\<user>\.llm-relay\bin\lane-launch.ps1 C:\Users\<user>\AppData\Local\agy\bin\agy.exe -p "Use the codebase-memory-mcp tools to list the indexed projects. Reply with the list only." --output-format json
```
...
`C:\Users\<user>\AppData\Local\Temp\claude\C--Code-llm-relay\4ede2635-ba94-44c2-8889-b106e098c155\scratchpad\dispatch-research\`
...
| file | change | backup |
|---|---|---|
| `C:\Users\<user>\.llm-relay\bin\lane-launch.ps1` | **new** — the no-window launcher | n/a |
| `C:\Users\<user>\.llm-relay\config.json` | 12 agy `cli` rungs routed through the launcher | `config.json.bak-2026-08-27-pre-lane-launch` |
| `C:\Users\<user>\.gemini\antigravity-cli\settings.json` | allow list repaired to the 4 valid non-shell categories | `settings.json.bak-2026-08-27-pre-vocab-fix` |
| `C:\Users\<user>\.llm-relay\config.json` | `--print-timeout 30m` plus a launcher `--timeout 2100` backstop on the same 12 rungs | `config.json.bak-2026-08-27-pre-agy-timeout` |
| `C:\Users\<user>\.claude\CLAUDE.md` | agy block rewritten: verified vocabulary, window cause, timeout default | `CLAUDE.md.bak-2026-08-27-pre-agy-vocab` |
