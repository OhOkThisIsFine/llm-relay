...
- **`rotate <id>`** — new ciphertext, same id, set `rotatedAt`, then clear the credential-fault state the old key produced: `breaker.clearCredentialFaults(credentialId)` and a **narrowed** `clearFacts(provider, credentialId, { kinds: ["credential-invalid"] })`.
  ...
  **Amendment 2026-08-24 (custody sprint, Packet 3 review):** as shipped, `add` STORES AND WARNS on a shadow instead of refusing — refusing would make the intended migration (store the key, then unset the env var) impossible, and `add` clears no learned state, so the refusal's original justification (rotation deleting facts while changing nothing on the wire) does not apply to it. `rotate` refuses exactly as specified above. The shipped `list` columns are provider, credential id, source, fingerprint, added/rotated, expiry, and lifecycle state; last-known status and headroom band remain on `/candidates`, which is that data's home.
...
**Amendment 2026-08-24 (custody sprint, Packet 3 review):** one narrow carve-out ships with the lifecycle CLI: `keys rotate` MAY call the existing token-gated `POST /cooldowns/clear` with the validated `kinds: ["credential-fault"]` narrowing, because rotation's clearing obligation reaches breaker state that exists only in the live process and no CLI-local write can touch. The rule this section exists for is intact: no endpoint accepts, returns, or transits secret material, and no new HTTP surface was added — the carve-out reuses M3's admission boundary and carries only provider/label selectors and non-secret cell identifiers.
